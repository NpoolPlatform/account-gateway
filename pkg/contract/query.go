...
func (h *queryHandler) formalize() {
	for _, info := range h.infos {
		...
		h.accs = append(h.accs, &npool.Account{
			...
			EntID:                info.EntID,
			GoodID:               info.GoodID,
			DelegatedStakingID:   info.DelegatedStakingID,
			...
			ContractOperatorType: info.ContractOperatorType,
...
func (h *Handler) GetAccount(ctx context.Context) (*npool.Account, error) {
	info, err := contractmwcli.GetAccount(ctx, *h.EntID)
	if err != nil {
		return nil, err
	}
	if info == nil {
	...
	handler := &queryHandler{
		Handler: h,
		infos:   []*contractmwpb.Account{info},
		coins:   map[string]*coinmwpb.Coin{},
		goods:   map[string]*goodmwpb.Good{},
	}
...
func (h *Handler) GetAccounts(ctx context.Context) ([]*npool.Account, uint32, error) {
	infos, total, err := contractmwcli.GetAccounts(
		ctx,
		&contractmwpb.Conds{},
		h.Offset,
		h.Limit,
